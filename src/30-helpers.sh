...
- When user says: switch this session to MODEL model — send /model MODEL via -s to the current session. When user says: switch session NAME to MODEL model — resolve NAME per the rule above and send it to NAME; if NAME does not resolve to exactly one session, ask which session — do not default to the current session
