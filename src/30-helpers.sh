...
- claude-mux may inject bracketed [claude-mux ...] notices (a daily tip, an \"update available\" notice, or a \"Claude Code was upgraded\" notice) into your turn context via a hook, wrapped in <assistant-must-display> tags. When present, surface the notice to the user verbatim at the START of your reply, before answering their request — do not paraphrase or drop it. Mention each notice at most once per session: an actionable notice (update / upgrade) re-appears every turn until you act on it, so once you have told the user this session, do not repeat it; it clears on its own when they act (update claude-mux / restart the session).
...
- When user says: switch this session to MODEL model — send /model MODEL via -s to the current session. When user says: switch session NAME to MODEL model — resolve NAME per the rule above and send it to NAME; if NAME does not resolve to exactly one session, ask which session — do not default to the current session. Before sending, only rewrite MODEL if it is a family-plus-version shorthand (a family like \`opus\`/\`sonnet\`/\`haiku\` followed by a version like \"4.8\", \"4-8\", or \"4 8\"): turn it into the dash-joined, \`claude-\`-prefixed form \`claude-<family>-<major>-<minor>\` (e.g. \"opus 4.8\" becomes \`claude-opus-4-8\`). Otherwise pass MODEL through unchanged: a bare family alias (\`opus\`), an already-full ID (\`claude-opus-4-8\`), and a date-suffixed ID (\`claude-haiku-4-5-20251001\`) are all already valid. Claude Code validates the result.
