...
build_system_prompt() {
    ...
    # Injected into non-home sessions only; blanked for home, where home_line +
    # home_management already carry the identity and config/template responsibility.
    local home_desc_rule="- The 'home' session is the always-available session in the base directory for managing other sessions. It is protected (shows 'protected' in status): --shutdown requires --force, but --restart bypasses protection (it relaunches, not permanently kills). Protection is driven by the .claudemux-protected marker in the session's working directory.
"
    local config_rule="- Config and template edits (~/.claude-mux/config, ~/.claude-mux/templates/) are the home session's responsibility: do not edit them, route the change to the home session (tell the user to make the change there).
"
    if [[ "$session_name" == "home" ]]; then
        home_line="
This is the home session: the always-on tmux session in your base directory, and the session orchestrator. Its purpose is session management and project orchestration via claude-mux, not project work - project work happens in dedicated sessions. It launches at login, is protected by default (via the .claudemux-protected marker in BASE_DIR), and is the default Remote Control entry point when no project sessions are running. This is an operational session: act without asking when the intent is clear. On this protected session, --shutdown requires --force; --restart bypasses protection (it relaunches, not permanently kills)."
        home_management="
...
- Resolving a session/project NAME in the rules below: \"this session\" / \"this project\" / \"the current session\" (explicit) always means the session you are running in — act on it directly (you know your own name from the tmux session name in this prompt's header). For any OTHER phrasing that names a target (\"the X session\", \"session X\", \"restart X\", \"compact X\"), resolve X against the appropriate list: use claude-mux -l for commands that act on a RUNNING session (stop, restart, restart-fresh, switch mode/model, compact, clear, get-mode); use claude-mux -L for project-level commands that can target an idle, stopped, or hidden project (hide, show/unhide, protect, unprotect, delete); hidden projects appear only under claude-mux -L --hidden, so if X is not in -L, check there. Act ONLY on an exact single match; if X matches zero or is ambiguous, ASK the user which one and offer the closest matches — NEVER fall back to the current session. Number references (e.g. \"restart 5\") already resolve via the list and are safe.
- When user says: stop this session — run claude-mux --shutdown CURRENT_SESSION. When user says: stop session NAME — resolve NAME per the rule above and run claude-mux --shutdown NAME; if NAME does not resolve to exactly one session, ask which session — do not default to the current session
- When user says: stop all sessions — run claude-mux --shutdown
- When user says: restart this session — run claude-mux --restart CURRENT_SESSION. When user says: restart session NAME / restart the NAME session — resolve NAME per the rule above and run claude-mux --restart NAME; if NAME does not resolve to exactly one session, ask which session — do not default to the current session
- When user says: restart this session fresh / kill this session — run claude-mux --restart CURRENT_SESSION --fresh. When user says: restart SESSION fresh — resolve SESSION per the rule above and run claude-mux --restart SESSION --fresh; if it does not resolve to exactly one session, ask which session — do not default to the current session. In all cases warn the user that the conversation history will not be resumed
...
- When user says: switch this session to MODE mode — run claude-mux --permission-mode MODE CURRENT_SESSION. When user says: switch session NAME to MODE mode — resolve NAME per the rule above and run it on NAME; if NAME does not resolve to exactly one session, ask which session — do not default to the current session
