...
# context — the only delivery path proven to surface in Remote Control. The tip is
# gated once-per-day per session (keyed on the session_id from the hook's stdin JSON);
# the actionable notices are persist-while-relevant (re-injected every prompt while
# their condition holds, no burn-on-emit gate). Must return fast and never block on
# the network: the GitHub API call is delegated to a disowned --update-check-bg process.
on_prompt() {
    ...
    # per-session tip_date ("_" marks empty; sid is "_" when missing/invalid). The
    # actionable notices keep no per-session state (persist-while-relevant), so only
    # tip_date is read. Always prints exactly 3 fields so the is_handshake flag
    # survives even when session_id is absent.
    local _sid _is_handshake _tip_date
    read -r _sid _is_handshake _tip_date < <(/usr/bin/python3 -c '
...
if sid != "_":
    try:
        ...
        pass
print(sid, is_handshake, tip_date)
' "$_state_dir" 2>/dev/null)
    ...
    local _out="" _new_tip_date="$_tip_date"
    ...
    # Persist per-session state (just tip_date) only if it changed. The actionable
    # notices keep no per-session state — they are persist-while-relevant, gated on a
    # live condition rather than a stamp.
    if [[ "$_new_tip_date" != "$_tip_date" ]]; then
        mkdir -p "$_state_dir" 2>/dev/null
        /usr/bin/python3 -c 'import json,sys
json.dump({"tip_date": sys.argv[2]}, open(sys.argv[1],"w"))' \
            "$_state_file" "$_new_tip_date" 2>/dev/null || true
    fi
