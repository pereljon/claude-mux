...
on_prompt() {
    ...
    # One python3 call parses the hook's stdin JSON: session_id (validated as a safe
    # filename token), whether this is the synthetic "Ready?" handshake, and the
    # per-session state file (tip_date, update_notify, notify_version — "_" marks
    # empty; sid is "_" when missing/invalid). notify_version lets a brand-new release
    # re-notify even inside the 7-day window. Always prints exactly 5 fields so the
    # is_handshake flag survives even when session_id is absent.
    local _sid _is_handshake _tip_date _update_notify _notify_version
    read -r _sid _is_handshake _tip_date _update_notify _notify_version < <(/usr/bin/python3 -c '
import json, sys, os, re
try: obj = json.load(sys.stdin)
except Exception: obj = {}
sid = obj.get("session_id", "") or ""
is_handshake = "1" if (obj.get("prompt", "") or "").strip() == "Ready?" else "0"
...
if sid != "_":
    try:
        ...
        pass
print(sid, is_handshake, tip_date, un, nv)
