...
### Fixed
- **Injection hardening from a full-injection audit against the new checklist.** A Fable audit of the entire injection surfaced a class of bugs where a rule's example was not runnable as written; fixes: (1) the "stop/restart this session" rules said "run `claude-mux --shutdown` for the current session" without the session argument, and bare `--shutdown`/`--restart` target **all** sessions — a literal first-read execution would stop everything; every "this session" self-target rule now uses the explicit `CURRENT_SESSION` placeholder. (2) The NAME-resolution rule said to resolve project-level targets with `-L (add --hidden)`, but `--hidden` lists **only** hidden projects, hiding normal ones from the lookup; reworded to check `-L` first, then `-L --hidden` only if not found. (3) The `--permission-mode MODE` "this session" rule likewise had no session arg (bare form errors). (4) Removed three redundant list rules (`list idle/stopped/running`) fully subsumed by the generic `list <STATUS>` rule. (5) The home session no longer gets the generic home-description and the shared config-ownership rule injected twice; both are now non-home-only, and the home block's identity line carries the protection mechanics. `CLAUDE_MUX_BIN` is now absolutized at generation so the injected `claude-mux path:` is always absolute. Injection-only change; takes effect per session after restart.
- **More injection-rule cleanups from the same audit.** `-s` was taught in three overlapping places (a self rule, the peer block, and a footer); consolidated into one cross-session `-s` rule plus the peer block, footer dropped. The "update claude-mux" rule no longer implies an on-demand "check latest" command that does not exist; it points at the prompt header (installed version) and the "update available" notice (latest). Protect/unprotect are now worded as project-level (they can target a stopped project), matching hide/show/delete.

### Removed
- **`ALLOW_CROSS_SESSION_CONTROL` config flag (and its `--cross-session-control` install option).** It was documented as restricting `-s` to the calling session when `false`, but was **never enforced** anywhere in the code (the send path only checks managed + active + slash + no-newline), so cross-session `-s` already worked regardless of the setting. Cross-session `-s` is in fact required by shipped behavior — the "compact/clear/switch the X session" triggers and home orchestration all send `-s` to a named peer — so the flag was both vestigial and self-contradictory. Removed the flag, its install option, the `config.example` entry, the startup warning, and the docs; `-s` targeting any managed session is now documented as intended. No behavior change (the flag did nothing); a leftover `ALLOW_CROSS_SESSION_CONTROL=` line in an existing config is harmless (ignored).
