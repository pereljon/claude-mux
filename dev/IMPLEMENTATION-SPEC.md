...
**Home-only block (v2.1.0 home-prompt-split):** when `session_name == "home"`, the prompt additionally carries the home-orchestrator identity (always-on, protected, the session orchestrator: session management and project orchestration, not project work; operational posture — act without asking when intent is clear) and the home self-management triggers (show/set config, add/edit/delete templates, marker conventions). The identity ships in the injection precisely so an ancestor `CLAUDE.md` in the base directory does not have to carry it — ancestor loading would leak it into every project session underneath. Gate is the session *name* only (`home`); the recorded name-vs-directory boundary lives in `dev/features/home-prompt-split.md`. Conversely, two shared rules are made **non-home-only** via `home_desc_rule` / `config_rule` (blanked when `session_name == "home"`): the generic "the 'home' session is..." description and the "config/template edits are home's responsibility, route them there" rule. Home already carries both facts (identity + protection mechanics in the home block, config/template ownership in the self-management triggers), so injecting the shared versions into home duplicated them; and the shared config rule's old "if this session is named 'home', edit directly" clause was dead text in every non-home session. The home block's identity line now also carries the protection mechanics (`--shutdown` needs `--force`, `--restart` bypasses) that previously lived only in the shared description.
