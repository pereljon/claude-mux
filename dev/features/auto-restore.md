...
- New config var `AUTORESTORE` → `config.example`, `config_help()`, settings table in `dev/IMPLEMENTATION-SPEC.md`.
- New marker `.claudemux-running` → marker registry in `dev/CODEMAP.md`, `ensure_gitignore_entry()`.
