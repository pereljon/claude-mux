...
**Severity:** Medium (the model switch silently stalls until the user manually presses a key)
**Status:** Open — **TOP-PRIORITY / active build target (promoted 2026-07-17), targeting v2.0.14.** Observed live THREE times: TWICE on `home` (2026-06-21) and again on `robotech-game-demo` (2026-07-17, manually cleared with one Enter). Designed + architect-reviewed in `dev/features/model-switch-confirm.md`; build plan + test plan (`model-switch-confirm-tests.md`) added 2026-07-17.
**Observed:** In the `home` session (v2.0.13), "change model to opus" correctly resolved to `claude-opus-4-8` and the session's Claude sent `claude-mux -s home '/model claude-opus-4-8'`. But `/model` on a **cached conversation** pops a confirmation dialog ("Switch model? … This conversation is cached for the current model. Switching to Opus 4.8 means the full history gets re-read … ❯ 1. Yes, switch to Opus 4.8 / 2. No, go back") that blocks until a keypress. The Claude that sent the command had already ended its turn ("Model switch sent"), so nothing confirmed the dialog and the session sat stuck on it until the dialog was manually `Enter`-ed (selecting the pre-highlighted "Yes").
**Root cause:** model switching goes through `claude-mux -s SESSION '/model <id>'`, which types the command into the pane and returns — **fire-and-forget**. When `/model` raises a confirmation dialog (only when the conversation is cached for the current model), there is no step that detects and confirms it. The dialog appears in the pane *after* the send, which the sending Claude cannot observe in-turn (the same in-turn-observability constraint that killed the `/model` picker fallback — see `model-resolution-notice-cleanup.md`). Not specific to v2.0.13 (the resolution worked); it is a gap in `/model` *delivery* for any cached-conversation switch.
