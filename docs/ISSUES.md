...
**Status:** Resolved in v2.0.14 (`confirm_model_switch`) — the `-s SESSION '/model <id>'` send now backgrounds a detached recognize-then-confirm confirmer that auto-answers the cached "Switch model?" dialog. Code complete + code-reviewed (0 CRITICAL/HIGH; the single-confirmer `mkdir` lock closed the one MEDIUM double-key race). Design/test: `dev/features/model-switch-confirm.md` + `-tests.md`. **One pre-release verification outstanding:** Phase 3.1 ship-gate — a live in-session SELF-switch (`-s SELF '/model …'` as Claude's own Bash tool call) that also trips the "Background work is running" exit-guard, to confirm the `( … & )` detach survives it. See CHANGELOG [2.0.14] and git history for the resolved detail.
