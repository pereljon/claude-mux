...
各プロジェクトの `.claude/settings.local.json` にある Claude Code の Stop hook が、会話ターンごとに `claude-mux --tipotd` を呼び出します。このコマンドは今日既にヒントを表示したか (`~/.claude-mux/.tip-date` で確認) をチェックします。表示済みなら約 6ms で終了します。未表示なら、ヒントを出力して今日の日付を記録します。

ヒントはデフォルトで有効です (`TIP_OF_DAY=true`)。任意のセッション内で「enable tips」または「disable tips」で切り替えられます。`TIP_MODE=daily` は一日中同じヒントを表示し、`TIP_MODE=random` は呼び出しごとにランダムなヒントを選びます (Stop hook の場合、日次ゲートにより一日一回のランダムヒントになります)。

`--tip` コマンドは日次ゲートに関係なく常に動作するため、いつでも「tip」と言えます。
...
| `~/.claude-mux/.tip-date` | 最後にヒントを表示した日付 |
