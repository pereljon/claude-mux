...
[English](../docs/FAQ.md) · [Español](FAQ.es.md) · [Français](FAQ.fr.md) · [Deutsch](FAQ.de.md) · [Português](FAQ.pt-BR.md) · [日本語](FAQ.ja.md) · **한국어** · [Italiano](FAQ.it.md) · [Русский](FAQ.ru.md) · [中文](FAQ.zh-CN.md) · [עברית](FAQ.he.md) · [العربية](FAQ.ar.md) · [हिन्दी](FAQ.hi.md)
...
config의 `DEFAULT_PERMISSION_MODE`으로 모든 프로젝트의 기본값을 설정하세요. 실행 중인 세션을 전환하려면 "이 세션을 plan 모드로 전환해"(또는 다른 모드 이름)라고 말하면 됩니다. "yolo"는 `bypassPermissions`의 별칭입니다.
...
- **Clear** ("이 세션을 클리어해"): 세션에 `/clear`를 전송합니다. 대화 기록을 지우고 새로 시작합니다. 세션은 계속 실행됩니다.
- **Compact** ("이 세션을 압축해"): 세션에 `/compact`를 전송합니다. 대화를 짧은 컨텍스트로 요약하여 컨텍스트 윈도우를 확보합니다. 기록은 압축된 형태로 보존됩니다.
- **Restart** ("이 세션을 재시작해"): Claude를 종료하고 `claude -c`로 다시 시작하여 마지막 대화를 재개합니다. 권한 모드 변경 후나 Claude가 멈췄을 때처럼 깨끗한 프로세스가 필요할 때 사용하세요.
...
템플릿 만들기: "이걸 web이라는 이름의 템플릿으로 저장해" (현재 프로젝트의 CLAUDE.md를 `~/.claude-mux/templates/web.md`로 복사합니다).

템플릿 사용하기: `claude-mux -n ~/projects/my-app --template web` 또는 세션 안에서: "web 템플릿을 사용해서 my-app이라는 새 프로젝트를 만들어".

템플릿 목록 보기: "list templates" 또는 `claude-mux --list-templates`.

## 오늘의 팁은 어떻게 동작하나요?

각 프로젝트의 `.claude/settings.local.json`에 있는 Claude Code `UserPromptSubmit` 훅이 프롬프트마다 `claude-mux --on-prompt`를 호출합니다. 그날의 첫 프롬프트는 대화에 팁 하나를 주입하고, 그날 이후의 프롬프트는 아무것도 주입하지 않습니다. 상태는 세션별이며 `~/.claude-mux/tip-state/<session_id>.json`에 저장되므로 각 활성 세션은 하루에 한 번 팁을 표시합니다. 이 훅은 컨텍스트에 주입하므로(출력이 트랜스크립트에만 가는 Stop 훅과 달리) 팁이 대화와 Remote Control에서 보입니다.

팁은 기본적으로 활성화되어 있습니다(`TIP_OF_DAY=true`). 세션 안에서 "팁 활성화" 또는 "팁 비활성화"로 전환하세요. `TIP_MODE=daily`는 하루 종일 같은 팁을 보여주고, `TIP_MODE=random`은 무작위 팁을 선택합니다.

`--tip` 명령은 일일 게이트와 관계없이(그리고 `TIP_OF_DAY`와 관계없이) 항상 동작하므로 언제든 "tip"이라고 말할 수 있습니다.
...
| `~/.claude-mux/tip-state/<session_id>.json` | 세션별 팁 날짜 + 업데이트 알림 스로틀 |
| `~/.claude-mux/.update-check` | 캐시된 버전 확인 결과 |
| `~/.claude-mux/.update-checking` | 백그라운드 업데이트 확인 중 잠금 |
...
## Claude Cowork Dispatch와 어떻게 다른가요?

Dispatch는 Claude 데스크톱 앱에서 작업을 시작하지만 앱이 실행 중이어야 하고 특정 프로젝트에 바인딩되지 않습니다. claude-mux는 영구적이고 프로젝트에 바인딩된 세션을 관리하며, 재부팅 후에도 유지되고 Remote Control을 통해 어디서나 접근할 수 있습니다 - 데스크톱 앱이 필요 없습니다.
...
세션 안에서 "이 프로젝트를 숨겨"라고 말하거나 `claude-mux --hide my-project`를 실행하세요. `.claudemux-ignore` 마커 파일이 생성됩니다. 프로젝트가 `claude-mux -L` 출력에 나타나지 않습니다. 숨겨진 프로젝트를 보려면: `claude-mux -L --hidden`. 숨김 해제: "이 프로젝트를 보여줘" 또는 `claude-mux --show my-project`.
...
기본적으로는 안 됩니다. Claude Code는 RC 세션에서 슬래시 명령(`/model`, `/clear` 등)을 지원하지 않습니다. claude-mux는 각 세션에 `claude-mux -s`를 주입하여 Claude가 tmux를 통해 자기 자신에게 슬래시 명령을 보낼 수 있도록 우회합니다. "Haiku로 전환해"나 "이 세션을 압축해"라고 말하면 Claude가 처리합니다.
