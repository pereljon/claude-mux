...
- चल रहे RC सेशन में भी slash commands काम नहीं करते - न model switching, न compacting, न permission mode बदलना
...
Claude: सभी प्रोजेक्ट्स में tip-of-the-day hook रजिस्टर या हटाता है
...
binary डाउनलोड करता है, `~/bin` में इंस्टॉल करता है, `PATH` में जोड़ता है, और interactive setup चलाता है। macOS और Linux पर काम करता है (Linux: LaunchAgent चरण छोड़ दिया जाता है)।

अपडेट करने के लिए:

```bash
claude-mux --update     # किसी भी सेशन के अंदर से या टर्मिनल से काम करता है
```

### Homebrew (macOS विकल्प)
...
interactive setup पूछता है कि आपके Claude प्रोजेक्ट्स कहाँ रहते हैं, लॉगिन पर होम सेशन शुरू करना है या नहीं, और कौन सा मॉडल उपयोग करना है। यह `~/.claude-mux/config` बनाता है और LaunchAgent install करता है।
...
प्रति-प्रोजेक्ट state केंद्रीय config में नहीं, बल्कि प्रोजेक्ट root पर मार्कर फ़ाइलों में संग्रहीत होती है। मार्कर `.claudemux-` prefix का उपयोग करते हैं और git-tracked प्रोजेक्ट में बनाए जाने पर स्वचालित रूप से `.gitignore` में जोड़ दिए जाते हैं।
...
```bash
claude-mux --hide                    # वर्तमान सेशन का प्रोजेक्ट -L लिस्टिंग से छिपाएं
claude-mux --hide my-project         # किसी विशिष्ट सेशन का प्रोजेक्ट छिपाएं
claude-mux --show my-project         # प्रोजेक्ट फिर से दिखाएं
...
claude-mux --delete my-project       # प्रोजेक्ट फ़ोल्डर को सिस्टम ट्रैश में ले जाएं (macOS)
```

मार्कर प्रोजेक्ट फ़ोल्डर के साथ rename और move पर भी बने रहते हैं। एक ही `.gitignore` पैटर्न (`.claudemux-*`) सभी वर्तमान और भविष्य के मार्कर को कवर करता है।

## कॉन्फ़िगरेशन

`~/.claude-mux/config` `claude-mux --install` द्वारा बनाया जाता है (या किसी भी command के पहले run पर यदि कोई config नहीं है)। किसी भी default को override करने के लिए इसे edit करें - script को कभी सीधे modify करने की ज़रूरत नहीं।
...
~/Claude/
├── work/
│   ├── project-a/          # ✓ .claude/ है - managed
│   │   └── .claude/
│   ├── project-b/          # ✓ .claude/ है - managed
│   │   └── .claude/
│   └── -archived/          # ✗ बाहर (- से शुरू)
│       └── .claude/
├── personal/
│   ├── project-c/          # ✓ .claude/ है - managed
│   │   └── .claude/
│   ├── .hidden/            # ✗ बाहर (hidden डायरेक्टरी)
│   │   └── .claude/
│   └── project-d/          # ✗ कोई .claude/ नहीं - Claude प्रोजेक्ट नहीं
├── deep/nested/project-e/  # ✓ .claude/ है - किसी भी गहराई पर मिला
│   └── .claude/
└── ignored-project/        # ✗ बाहर (.claudemux-ignore)
    ├── .claude/
    └── .claudemux-ignore
```
...
होम सेशन को अतिरिक्त context मिलता है: इसकी भूमिका का विवरण, और config तथा templates पढ़ने/संपादित करने के लिए self-management triggers। जब `ALLOW_CROSS_SESSION_CONTROL=true` हो, तो send command किसी भी सेशन को target कर सकती है, सिर्फ़ खुद को नहीं। path launch समय पर script का absolute path है, इसलिए सेशन `PATH` पर निर्भर नहीं करते।
...
# प्रोजेक्ट मार्कर (सभी commands सेशन नामों का उपयोग करते हैं, paths का नहीं)
claude-mux --hide                # वर्तमान सेशन का प्रोजेक्ट -L लिस्टिंग से छिपाएं
claude-mux --hide my-project     # सेशन नाम से विशिष्ट प्रोजेक्ट छिपाएं
claude-mux --show my-project     # प्रोजेक्ट फिर से दिखाएं
claude-mux --protect             # इस सेशन को गलती से बंद होने से बचाएं
claude-mux --unprotect           # सुरक्षा हटाएं
claude-mux --delete my-project           # प्रोजेक्ट फ़ोल्डर सिस्टम ट्रैश में भेजें (macOS)
claude-mux --delete my-project --yes     # वही, confirmation prompt छोड़ें
claude-mux --rename my-project new-name  # प्रोजेक्ट डायरेक्टरी का नाम बदलें
claude-mux --move my-project ~/Claude/work  # प्रोजेक्ट को नए parent में ले जाएं
...
claude-mux --config-help         # defaults और विवरण के साथ सभी config विकल्प दिखाएँ
...
टर्मिनल से चलाने पर, output real time में stdout पर mirror होता है। LaunchAgent के माध्यम से चलाने पर, output केवल log file में जाता है।
...
### सेशन शुरू होने पर "Session ready!"

जब कोई सेशन शुरू होता है या पुनः शुरू होता है, claude-mux Claude के load होने के बाद स्वचालित रूप से एक `Ready?` message भेजता है। injection Claude को "Session ready!" और कुछ नहीं के साथ respond करने के लिए कहता है। यह confirm करता है कि सेशन alive है और injection काम कर रहा है।

### Remote Control पर slash commands

Slash commands (जैसे `/model`, `/clear`) RC सेशन में [मूल रूप से supported नहीं हैं](https://github.com/anthropics/claude-code/issues/30674)। claude-mux इसका समाधान करता है - प्रत्येक सेशन को `claude-mux -s` inject किया जाता है ताकि Claude tmux के माध्यम से खुद को slash commands भेज सके।
...
- [FAQ](../FAQ.md) - claude-mux के बारे में आम सवाल
- [ज्ञात समस्याएँ](../ISSUES.md) - open bugs, नियोजित features, और हल की गई समस्याएँ
