...
여러 프로젝트에서 Claude Code를 사용할 때 불편한 점이 있습니다:

- 터미널을 닫으면 세션이 종료됨
- Remote Control 세션에서는 `/model`이나 `/compact` 같은 슬래시 명령을 실행할 수 없음
- 아직 실행되지 않은 프로젝트의 세션을 쉽게 시작할 수 없음
- 휴대폰에서 모델, 권한 모드를 바꾸거나 컨텍스트를 압축할 수 없음
