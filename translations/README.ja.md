...
複数プロジェクトをまたいで Claude Code を使うと摩擦が生じます:

- ターミナルを閉じるとセッションが終了する
- Remote Control セッションでは `/model` や `/compact` などのスラッシュコマンドを実行できない
- まだ起動していないプロジェクトのセッションを簡単に開始できない
- スマートフォンからモデルの切り替え、permission モードの変更、コンテキストのコンパクト化ができない
...
home セッションは常に**保護**されています。`--shutdown home` は `--force` なしでは停止を拒否します。保護されたセッションはステータス出力で `*` 付き (例: `active*`) として表示されます。
...
`~/.claude-mux/config` は初回実行時に自動的に作成されます。LaunchAgent のセットアップ（ログイン時のホームセッション）には、リポジトリをクローンして `install.sh` を実行してください。
