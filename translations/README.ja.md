...
すべてのプロジェクトに対して永続的な Claude Code セッションを提供し、Claude モバイルアプリからどこでもアクセスできるようにします。***Managed by Claude!***
...
- **マルチ CLI ツール対応** -- `AGENTS.md` と `GEMINI.md` を `CLAUDE.md` へのシンボリックリンクとして作成し、Codex CLI、Gemini CLI などのツールが同じ指示を共有
- **権限の自動承認** -- claude-mux を各プロジェクトの許可リストに追加し、Claude がプロンプトなしでセッションコマンドを実行可能に
...
バイナリをダウンロードして `~/bin` にインストールし、`PATH` に追加して対話型セットアップを実行します。macOS と Linux で動作します (Linux では LaunchAgent のステップはスキップされます)。

更新するには:

```bash
claude-mux --update     # 任意のセッション内から、またはターミナルから実行可能
```

### Homebrew (macOS 向け代替手段)
...
| ステータス | 意味 |
|--------|---------|
| `running` | tmux セッションが存在し、Claude が動作中 |
...
すでに動作中のセッションがあるディレクトリで `claude-mux` を実行すると、そのセッションにアタッチします。複数のターミナルから同じセッションにアタッチできます (標準的な tmux の動作)。
...
```bash
claude-mux --hide                    # 現在のセッションのプロジェクトを -L 一覧から非表示にする
claude-mux --hide my-project         # 特定のセッションのプロジェクトを非表示にする
claude-mux --show my-project         # プロジェクトの非表示を解除する
claude-mux --protect                 # このセッションを誤ったシャットダウンから保護する
claude-mux --unprotect               # 保護を解除する
claude-mux -L --hidden               # 非表示のプロジェクトのみ一覧表示する
claude-mux --delete my-project       # プロジェクトフォルダをシステムのゴミ箱に移動 (macOS)
```
...
home セッションには追加のコンテキストが注入されます: その役割の説明と、設定やテンプレートの読み書きのためのセルフ管理トリガーです。`ALLOW_CROSS_SESSION_CONTROL=true` の場合、send コマンドは自身だけでなく任意のセッションを対象に取れます。パスは起動時のスクリプトの絶対パスで、セッションは `PATH` に依存しません。
...
claude-mux -n ~/app --template web        # 特定の CLAUDE.md テンプレートで新規プロジェクト
claude-mux -n ~/app --no-multi-coder      # AGENTS.md/GEMINI.md シンボリックリンクなしで新規プロジェクト

# セッション管理
claude-mux -l                    # ステータス別にセッションを一覧 (active、running、stopped)
claude-mux -L                    # すべてのプロジェクトを一覧 (active + idle)
claude-mux -L --hidden           # 非表示のプロジェクトのみ一覧表示
...
# プロジェクトマーカー (すべてのコマンドはセッション名で操作、パスではない)
claude-mux --hide                # 現在のセッションのプロジェクトを -L 一覧から非表示にする
claude-mux --hide my-project     # セッション名で特定のプロジェクトを非表示にする
claude-mux --show my-project     # プロジェクトの非表示を解除する
claude-mux --protect             # このセッションを誤ったシャットダウンから保護する
claude-mux --unprotect           # 保護を解除する
claude-mux --delete my-project           # プロジェクトフォルダをシステムのゴミ箱に移動 (macOS)
claude-mux --delete my-project --yes     # 同上、確認プロンプトをスキップ
claude-mux --rename my-project new-name  # プロジェクトディレクトリをリネーム
claude-mux --move my-project ~/Claude/work  # プロジェクトを別の親ディレクトリに移動
...
### セッション開始時の「Session ready!」

セッションが起動または再起動すると、Claude の読み込み完了後に claude-mux が自動的に `Ready?` メッセージを送信します。注入された指示により、Claude は「Session ready!」とだけ返答します。これでセッションが生きており、注入が機能していることを確認できます。
...
- [FAQ](../FAQ.md) -- claude-mux に関するよくある質問
- [既知の問題](../ISSUES.md) -- 既知のバグ、予定されている機能、解決済みの問題
