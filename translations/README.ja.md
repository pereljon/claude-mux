...
home セッションは常に**保護**されています。`--shutdown home` は `--force` なしでは停止を拒否します。保護されたセッションはステータス列に `protected` と表示されます。呼び出し元セッションは名前列に `>` が付きます。
...
| `protected` | `running` と同様だが、セッションが保護されている — `--shutdown` には `--force` が必要。home セッションは常に保護されています。 |
