...
```bash
claude-mux --hide                    # 从 -L 列表中隐藏当前项目
claude-mux --show                    # 取消隐藏当前项目
claude-mux --protect                 # 保护此会话防止意外关闭
claude-mux --unprotect               # 移除保护
claude-mux -L --hidden               # 仅列出隐藏的项目
claude-mux --delete ~/projects/old   # 将项目文件夹移至系统回收站 (macOS)
```
...
# 项目标记
claude-mux --hide                    # 从 -L 列表中隐藏当前项目
claude-mux --hide ~/projects/old     # 隐藏特定项目
claude-mux --show                    # 取消隐藏当前项目
claude-mux --protect                 # 保护此会话防止意外关闭
claude-mux --unprotect               # 移除保护
claude-mux --delete ~/projects/old           # 将项目文件夹移至系统回收站 (macOS)
claude-mux --delete ~/projects/old --yes     # 同上，跳过确认提示
