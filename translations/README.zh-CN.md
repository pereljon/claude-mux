...
跨多个项目使用 Claude Code 存在摩擦：

- 关闭终端后会话就消失了
- Remote Control 会话无法运行 `/model`、`/compact` 等斜杠命令
- 无法为尚未运行的项目轻松启动会话
- 无法从手机切换模型、权限模式或压缩上下文
...
主会话始终是**受保护的** — `--shutdown home` 会拒绝在没有 `--force` 的情况下停止它。受保护的会话在状态输出中以 `*` 标记（例如 `active*`）。
