...
Работа с Claude Code в нескольких проектах создаёт неудобства:

- Сессии завершаются при закрытии терминала
- Сессии Remote Control не могут выполнять слэш-команды вроде `/model` или `/compact`
- Нельзя легко запустить сессию для проекта, который ещё не запущен
- Переключить модели, режимы разрешений или сжать контекст с телефона невозможно
...
Home-сессия всегда **защищена** — `--shutdown home` отказывается её останавливать без `--force`. Защищённые сессии помечаются `*` в выводе статуса (например, `active*`).
...
При первом запуске `~/.claude-mux/config` создаётся автоматически со всеми настройками в виде комментариев. Отредактируйте файл, чтобы переопределить значения по умолчанию — сам скрипт менять не нужно.
