...
Home-сессия всегда **защищена** — `--shutdown home` отказывается её останавливать без `--force`.
...
| `protected` | то же что `running`, но сессия защищена — для остановки через `--shutdown` требуется `--force`. Home-сессия всегда защищена. |
