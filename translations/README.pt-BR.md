...
A sessão home é sempre **protegida** — `--shutdown home` se recusa a pará-la sem `--force`.
...
| `protected` | igual a `running`, mas a sessão é protegida — `--shutdown` exige `--force` para parar. A sessão home é sempre protegida. |
