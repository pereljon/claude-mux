...
סשן הבית הוא תמיד **מוגן** — `--shutdown home` מסרב לעצור אותו ללא `--force`. סשנים מוגנים מוצגים עם `protected` בעמודת הסטטוס; הסשן הנוכחי מסומן ב-`>` בעמודת השם.
...
| `protected` | כמו `running`, אך הסשן מוגן — `--shutdown` דורש `--force` כדי לעצור אותו. סשן הבית מוגן תמיד. |
